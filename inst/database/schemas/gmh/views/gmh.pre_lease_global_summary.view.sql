DROP VIEW gmh.pre_lease_global_summary;

CREATE OR REPLACE VIEW gmh.pre_lease_global_summary AS
WITH base AS (
  SELECT
    s.report_date,
    ...
    (((s.approved_new_count + s.partially_completed_new_count + s.completed_new_count) +
      ...
      / s.available_count) AS current_preleased_percent,
    ...
    ((s.approved_count_prior + s.partially_completed_count_prior + s.completed_count_prior)::numeric
      / s.available_count) AS prior_preleased_percent,
    (((s.approved_new_count + s.partially_completed_new_count + s.completed_new_count) +
      (s.approved_renewal_count + s.partially_completed_renewal_count + s.completed_renewal_count))
      - (s.approved_count_prior + s.partially_completed_count_prior + s.completed_count_prior))
    ...
    (COALESCE(w.weekly_new, 0) + COALESCE(w.weekly_renewal, 0))::numeric / s.available_count
  ...
  LEFT JOIN entrata.pre_lease_weekly w ON s.property_id = w.property_id AND s.report_date = w.report_date
  ...
  WHERE s.report_date = (
    ...
    FROM entrata.pre_lease_summary_by_property
  )
)
SELECT
  report_date,
...
SELECT * FROM gmh.pre_lease_global_summary
